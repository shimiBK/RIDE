  ...
  const handleSendMessage = async (addresseeId) => {
    try {
      ...
      const res = await axios.get(
        `${SERVER_URL}/api/conversation/find/${addresseeId}/${user?._id}`
      );
      ...
      <div className={containerStyle}>
        {myRides && (
          <div className="infoItem">
            <div className="myRideEventName">
              {convertTitle(ride.eventName)}
            </div>
          </div>
        )}
        <div className="infoItem">
          <img
            src={ride.userImg ? ride.userImg : "/assests/blank-profile.png"}
            alt=""
        ...
        <span className="genderIcon">
          ...
          {ride.userGender === "Female" && (
            <FemaleIcon style={{ color: "#f15bb5" }} />
          )}
        </span>
        <button
          className="facebookBtn"
          onClick={() => {
            handleSendMessage(ride.userID);
          }}
        >
